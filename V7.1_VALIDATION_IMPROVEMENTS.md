...
## v7.0 ATR Stop Loss Clarification (Post-v7.1 Update)

**Third-Party Feedback (December 16, 2025):**
> "The ATR stop rule uses the word 'floor.' That's backwards. A 'floor' implies minimum loss, a 'cap' implies maximum loss. Call it a 'cap' and add the explicit formula: `stop_pct = -min(2.5*ATR/entry_price, 0.07)`"

**Terminology Fixed:**
- **Before**: "ATR-based stops with -7% maximum loss floor"
- **After**: "ATR-based stops with -7% maximum loss cap"

**Explicit Formula:**
```python
# ATR stop calculation (v7.0 implementation)
atr_stop_pct = -min(2.5 * ATR / entry_price, 0.07)

# Examples:
# Volatile stock (ATR = $6, Entry = $100):
#   ATR suggests: 2.5 * 6 / 100 = -15%
#   Cap applies: min(-15%, -7%) = -7% (TIGHTER, limits max loss)

# Stable stock (ATR = $2, Entry = $100):
#   ATR suggests: 2.5 * 2 / 100 = -5%
#   Cap not reached: min(-5%, -7%) = -5% (TIGHTER, respects natural volatility)
```

**Why This Matters:**
- The -7% is a **maximum loss cap**, not a minimum loss floor
- System uses the **tighter** (closer to entry) of the two values
- Volatile stocks: ATR protection capped at -7% to prevent excessive losses
- Stable stocks: Natural ATR stop used (e.g., -4%, -5%) for tighter risk control
- Formula enables unambiguous validation and backtesting

**Documentation Updated:**
- [TEDBOT_OVERVIEW.md:838-840](TEDBOT_OVERVIEW.md#L838-L840) - Changed "floor" to "cap" with explicit formula reference
- This section added to v7.1 validation documentation for completeness
