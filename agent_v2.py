#!/usr/bin/env python3
"""
Paper Trading Lab - Production Agent v2.0
Complete implementation with closed-loop learning

Features:
- CSV logging for completed trades
...
class TradingAgent:
    """Production-ready trading agent with complete learning loop"""
...
"""
    ...
    def validate_trade_decisions(self, claude_response):
        ...
        if not exclusions:
            return True, []  # No exclusions, all good
        
        # Try to extract catalyst types from Claude's response
    ...
    def extract_completed_trades_from_portfolio(self):
        ...
        # Look for closed_positions field (if it exists)
        ...
        if closed:
            ...
            # Remove closed positions from portfolio after logging
    ...
    def save_response(self, command, response_data):
        ...
        # Save to latest file for dashboard
        latest_file = self.project_dir / f'latest_{command}.json'
    ...
    def execute_go_command(self):
        ...
        # Validate decisions
        print("3. Validating trade decisions against learned rules...")
        is_valid, violations = self.validate_trade_decisions(
            response.get('content', [{}])[0].get('text', '')
        )
        ...
        if violations:
            ...
            for v in violations:
                print(f"      {v}")
            print("   Note: Review recommendations carefully\n")
        else:
            print("   ✓ All decisions validated\n")
        
        # Save response
        print("4. Saving response...")
    ...
    def execute_learn_command(self, tier='daily'):
        ...
        try:
            ...
            print("   ℹ Ensure learn_{tier}.py is in the project directory")
            return False
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Paper Trading Lab Agent v2.0")
