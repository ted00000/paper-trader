...
LOG_FILE="$SCRIPT_DIR/logs/analyze.log"
STATUS_FILE="$SCRIPT_DIR/dashboard_data/operation_status/analyze_status.json"
...
# Run ANALYZE with logging and error capture
echo "============================================================" >> "$LOG_FILE"
echo "ANALYZE Command Starting: $(date)" >> "$LOG_FILE"
echo "============================================================" >> "$LOG_FILE"

if python3 "$AGENT_SCRIPT" analyze >> "$LOG_FILE" 2>&1; then
    # Success
    update_status "SUCCESS"
    echo "ANALYZE command completed successfully: $(date)" >> "$LOG_FILE"
    exit 0
else
    # Failure
    EXIT_CODE=$?
    update_status "FAILED" "ANALYZE command failed with exit code $EXIT_CODE"
    echo "ANALYZE command failed with exit code $EXIT_CODE: $(date)" >> "$LOG_FILE"
    exit $EXIT_CODE
fi
